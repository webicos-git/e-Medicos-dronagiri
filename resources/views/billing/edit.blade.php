         ...
         <div class="card shadow mb-4">
            ...
            <div class="card-body">
               <div class="form-group">
                  ...
                  <input class="form-control" type="text" value="{{$billing->opd_no}}" name="opd_no" readonly>
               </div>
               <div class="form-group">
                  <label for="medicine">{{ __('sentence.Patient Name') }}</label>
                  <input class="form-control" type="text" value="{{ $billing->Patient->name }}" name="opd_no" readonly>
                  <input type="hidden" name="patient_id" value="{{ $billing->Patient->id }}">

                  {{-- <select class="form-control select2 select2-hidden-accessible" id="medicine" tabindex="-1" name="patient_id" aria-hidden="true">
                     <option selected readonly value="{{ $billing->Patient->id }}">{{ $billing->Patient->name }}</option>
               ...
               <div class="form-group">
                  ...
                  <select class="form-control" name="payment_mode" id="PaymentMode">
                     <option 
                     @if ($billing->payment_mode === 'Cash')
                        selected
                     @endif
                     value="Cash">{{ __('sentence.Cash') }}</option>

                     <option 
                     @if ($billing->payment_mode === 'UPI')
                        selected
                     @endif
                     value="UPI">{{ __('sentence.UPI') }}</option>
                  </select>
               </div>
               <div class="form-group">
                  ...
                  <select class="form-control" name="payment_status">
                     ...
                     @if ($billing->payment_status === 'Paid')
                        selected
                     @endif
                     value="Paid">{{ __('sentence.Paid') }}</option>
                     
                     <option 
                     @if ($billing->payment_status === 'Unpaid')
                        selected
                     @endif
                     value="Unpaid">{{ __('sentence.Unpaid') }}</option>
                     
                     <option 
                     @if ($billing->payment_status === 'Balance')
                        selected
                     @endif
                     value="Balance">{{ __('sentence.Balance/Pending') }}</option>
                  </select>
               </div>
               <div class="form-group">
